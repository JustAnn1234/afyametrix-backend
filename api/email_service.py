...
FROM_EMAIL = os.getenv("EMAIL_FROM", "AfyaMetrix <noreply@afyametrix.com>")
...
class EmailService:
    def __init__(self):
        ...
        print(f"   Resend API: {'Configured' if self.resend_key else 'Not configured'}")
        print(f"   SMTP Server: {self.server}:{self.port}")
        
        if self.resend_key:
            resend.api_key = self.resend_key
            print(f"✅ Resend is primary email provider")
        else:
            print(f"⚠️  No Resend API key found")

    async def send_email_resend(self, to_email: str, subject: str, html_content: str, text_content: str = None):
        """Send email using Resend API (Primary method)"""
        ...
        try:
            params = {
                "from": self.from_email,          # Uses EMAIL_FROM from .env
                "to": [to_email],
                "subject": subject,
                "html": html_content,
            }
            ...
            print(f"❌ Resend API failed: {str(e)}")
            return False

    async def send_email_smtp(self, to_email: str, subject: str, html_content: str, text_content: str = None):
        """Send email via SMTP (Backup only)"""
        try:
            if not self.username or not self.password:
                raise Exception("SMTP credentials not configured")
            
            print(f"📧 Attempting SMTP (backup) to {self.server}:{self.port}")
    ...
    async def send_email(self, to_email: str, subject: str, html_content: str, text_content: str = None):
        """Primary: Resend → Backup: SMTP"""
        
        print(f"📧 PRODUCTION: Sending email to {to_email}")
        
        # 1. Try Resend first (recommended)
        if self.resend_key:
            success = await self.send_email_resend(to_email, subject, html_content, text_content)
            if success:
                return True
        ...
        print(f"❌ All methods failed for: {to_email}")
    ...
    async def test_email_connection(self):
        """Simple health check"""
        if self.resend_key:
            return True   # Resend doesn't need connection test
        return False
    ...
    async def send_verification_email(self, email: str, verification_code: str, name: str):
        ...
        <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="padding: 30px 20px;">
                <h2>Hello {name}!</h2>
                <p>Welcome to AfyaMetrix! Please verify your email address.</p>
