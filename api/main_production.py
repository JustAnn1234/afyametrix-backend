...
def get_password_hash(password: str) -> str:
    ...
    # Explicitly enforce the bcrypt 72-byte limit before hashing
...
async def register_user(request: Request, user: UserRegister, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Hash password (will raise ValueError if > 72 bytes)
        try:
...
# def get_password_hash(password: str) -> str:
#     """Hash a password using bcrypt (safe for production)."""
#     # Explicitly enforce the bcrypt limit before hashing
#     if len(password.encode("utf-8")) > 72:
#         raise ValueError("Password cannot be longer than 72 bytes")
#     return pwd_context.hash(password)
